...
class ResidualLayer(tf.keras.Model):
    def __init__(self):
        ...
        self.conv = tf.keras.layers.Conv2D(filters=128,kernel_size=3,strides=1,padding='same',kernel_regularizer='l2')
        self.batch_norm = tf.keras.layers.BatchNormalization()
        self.conv2 = tf.keras.layers.Conv2D(filters=128,kernel_size=3,strides=1,padding='same',kernel_regularizer='l2')
...
class ConvolutionalStem(tf.keras.Model):
    def __init__(self):
        ...
        self.initial_cnn_layer = tf.keras.layers.Conv2D(filters=128,kernel_size=3,strides=1,padding='same',kernel_regularizer='l2',input_shape=(None,8,8,None))
        self.initial_batch_norm = tf.keras.layers.BatchNormalization()
        self.ReLU = tf.keras.layers.ReLU()
        self.residual_layers = [ResidualLayer() for _ in range(10)]
