...
class MctsNode:
    ...
    def add_loss(self):
        ...
        #although loss can go up to 127 in theory we cap it at twenty because it makes no sense to further increase it
        if loss <= 20:
...
class MctsTree:
    ...
    :attribute A (int or float): The constant used for the confidence bound when selecting the final move, default value is 1, must be greater than 0.
    :attribute c (int or float): The exploration constant used in the UCBT formula, default value is 2.
    '''

    def __init__(self, game : ChessGame, game_limit=1000, threads=6):
        '''
        ...
        self.NODE_LIMIT = game_limit * threads * 23 #The node limit is calculated as amount of nodes to create times threads times average move count for chess (~23) (500*6*23=69.000)
        self.THREADS = 6
        ...
        self.LIMITS[0] = (1,self.LIMITS[0][1]) #sets the limit for the first thread to start at index 1 to exclude the root node
    ...
    def close(self):
        ...
        try:
            ...
            print('Warning: Could not close buffer properly after calling MctsTree.close')
        try:
            if not self.file.closed:
                self.file.close()
        except:
            print('Warning: Could not close file properly after calling MctsTree.close')
    ...
    def evaluation(self):
        '''
        Method for retrieving the evaluation of the current position.
        Returns the value of the parent node which is the evaluation value.
        Note that due to the nature of this implementation it ranges from 1 to 0 and draw and loss are considered equal.
        '''

        return MctsNode(self.buf, 0).value
